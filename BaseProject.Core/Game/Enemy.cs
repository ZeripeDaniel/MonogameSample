		...
		IEnumerable<int> MoveRandomly()
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < 6; i++)
				{
					...
					var bounds = BaseProjectGame.Viewport.Bounds;
					bounds.Inflate(-image.Width / 2 - 1, -image.Height / 2 - 1);

                    // 적이 경계 밖에 있으면 가장자리에서 멀어지게 만듭니다.
                    if (!bounds.Contains(Position.ToPoint()))
						direction = (BaseProjectGame.ScreenSize / 2 - Position).ToAngle() + rand.NextFloat(-MathHelper.PiOver2, MathHelper.PiOver2);
